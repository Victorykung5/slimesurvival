...
void gamelogic::logic(sf::RenderWindow* window, sf::View view, slime* slime1, std::vector<knight*>& knight1, float deltatime, char* gamestate,float knightexp)
{
	//damage
	if (knight1.size() >= 1)
	{
		for (int i = 0; i < knight1.size(); i++)
		{
			...
			{
				if (slime1->damageable == true&&knight1[i]->knighttype!='n')
				{
			...
			for (int j = 0; j < 5; j++)
			{
				if (knight1.size() > 0) 
				{
					if (knight1[i]->knighthitbox.getGlobalBounds().intersects(slime1->shoot[j].shoot.getGlobalBounds()))
					{
						...
						slime1->shoot[j].shootbol = false;
						slime1->shoot[j].shoot.setRadius(0);
						slime1->shoot[j].shoot.setPosition(slime1->positionslime.x, slime1->positionslime.y);
						slime1->shoot[j].shootbol = false;
					}
				}
			}
